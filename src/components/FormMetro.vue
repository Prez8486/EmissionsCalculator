<template>
  <div class="form-container">
    ...
    <form @submit.prevent="calculateEmission">
      <label>Metro Distance Travelled (km):</label>
  ...
  export default {
    ...
    methods: {
      async calculateEmission() {
        ...
        try {
          this.loading = true;
          const res = await fetch("https://emissionscalculatorbackend.onrender.com/api/emissions/metro/emissions", {
            method: "POST",
          ...
          console.error(err);
          alert("Error calculating metro emissions");
        } finally {
          this.loading = false;
        }
      },
